...
Search finds pages. OpenNav makes built static pages usable by AI agents.

Run OpenNav after your existing static site build. It reads a finished folder
such as `dist/`, `out/`, `build/`, or `site/`, then writes agent-readable files
into that same folder: `llms.txt`, `llms-full.txt`, Markdown page mirrors,
`/.well-known/opennav.json`, HTML discovery links, and optional `robots.txt`
Content Signals guidance.

The result is a normal static site with a stable reading path for agents:
clean Markdown instead of visual HTML scraping, predictable discovery files
instead of layout-specific extraction, and compatibility metadata tools can
