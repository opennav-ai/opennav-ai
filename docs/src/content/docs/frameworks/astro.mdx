...
>
  <table data-opennav-responsive-table="skip">
    ...
    <tbody>
      ...
      <tr>
        <td>
          <code>contentExtraction</code>
        </td>
        <td>
          <code>{"{ stripLayout?: boolean }"}</code>
        </td>
        <td>
          Optional. Controls whether generated Markdown keeps the whole HTML
          body or strips the documented layout elements first.
        </td>
      </tr>
      <tr>
        <td>
...
For Astro server-rendered (SSR) pages, use `OpenNavServer` from
`@opennav-ai/opennav/server`. Import it in your route handler and pass the
incoming request and the rendered HTML response:
...
on-the-fly. Browser requests still receive HTML, and responses include
`Vary: Accept` so caches keep both representations correct.

For static Astro sites, continue using `OpenNavAstro()` as described above.
