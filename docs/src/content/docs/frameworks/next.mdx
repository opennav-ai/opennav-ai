...
For Next.js server-rendered pages, use `OpenNavServer` from
`@opennav-ai/opennav/server`. Call `negotiate()` in a Route Handler:
...
For static Next.js export sites, continue using `OpenNavNext()` as described
above.
